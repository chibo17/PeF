...
def gen_subsets(s):
    if len(s) == 0:
        ...
        subsets = []
        smaller_subsets = gen_subsets(s[1:])
...
def knapsack(weights, values, capacity):
    max_value = 0
    for s in gen_subsets(list(range(len(weights)))):
        total_weight = sum(weights[i] for i in s)
        total_value = sum(values[i] for i in s)
...
def gen_permutations(lst):
    if len(lst) == 0:
        ...
        for i in range(len(lst)):
            first = lst[i]
            rest = lst[:i] + lst[i+1:]
            for p in gen_permutations(rest):
